...
pub(crate) fn expand(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    for delim in decl.fields.content.into_iter() {
        ...
        if !custom_field_names.insert(name.to_string()) {
            return compile_error("picante: duplicate field in db struct");
        }
    ...
    for input in &args.inputs {
        ...
        for name in [&keys_field, &data_field] {
            ...
            if !generated_field_names.insert(name_s.clone()) || custom_field_names.contains(&name_s)
            {
                return compile_error("picante: db field name conflict");
            }
        }
    ...
    for interned in &args.interned {
        ...
        if !generated_field_names.insert(field_s.clone()) || custom_field_names.contains(&field_s) {
            return compile_error("picante: db field name conflict");
        }
    ...
    // Ensure custom fields don't overlap with our generated ones.
    for name in custom_field_names {
        if generated_field_names.contains(&name) {
            return compile_error("picante: db field name conflict");
        }
    }
    ...
    while let Some(tt) = it.next() {
        match tt {
            TokenTree::Ident(key) => {
                ...
                let items = parse_list(group.stream())?;

                match key.to_string().as_str() {
                    "inputs" | "input" => out.inputs = items,
                    "interned" => out.interned = items,
                    "tracked" => out.tracked = items,
...
fn parse_list(ts: TokenStream2) -> Result<Vec<ItemPath>, String> {
    let mut out = Vec::new();
